...
- **schema in:** the wire :class:`~elide_bento_core.ner.v1.Schema` →
...
- **result out:** gliner2's ``batch_extract`` result dict → the typed
  :class:`~elide_bento_core.ner.v1.NerResponse` groups (gliner2's ``confidence`` is
  exposed as ``score``).
"""
...
from elide_bento_core.ner.v1 import (
    Classification,
    ClassLabel,
    Entity,
    EntitySpec,
