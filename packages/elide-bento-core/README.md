...
- [`elide_bento_core.ocr.v1`](src/elide_bento_core/ocr/v1.py) — OCR contract
  (`Page → Block → Line → Word`, geometry as axis-aligned `BoundingBox` plus
  optional polygon).
- [`elide_bento_core.ocrvl.v1`](src/elide_bento_core/ocrvl/v1.py) — vision-language OCR
  contract (block-level regions with text, layout kind, bbox, and reading
  order).
- [`elide_bento_core.ner.v1`](src/elide_bento_core/ner/v1) — NER contract (`Entity` with
